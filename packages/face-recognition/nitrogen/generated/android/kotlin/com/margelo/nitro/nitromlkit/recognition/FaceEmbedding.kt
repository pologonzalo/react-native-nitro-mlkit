///
/// FaceEmbedding.kt
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

package com.margelo.nitro.nitromlkit.recognition

import androidx.annotation.Keep
import com.facebook.proguard.annotations.DoNotStrip
import java.util.Objects


/**
 * Represents the JavaScript object/struct "FaceEmbedding".
 */
@DoNotStrip
@Keep
data class FaceEmbedding(
  @DoNotStrip
  @Keep
  val vector: DoubleArray
) {
  /* primary constructor */

  override fun equals(other: Any?): Boolean {
    if (this === other) return true
    if (other !is FaceEmbedding) return false
    return Objects.deepEquals(this.vector, other.vector)
  }

  override fun hashCode(): Int {
    return arrayOf<Any?>(
      vector
    ).contentDeepHashCode()
  }

  companion object {
    /**
     * Constructor called from C++
     */
    @DoNotStrip
    @Keep
    @Suppress("unused")
    @JvmStatic
    private fun fromCpp(vector: DoubleArray): FaceEmbedding {
      return FaceEmbedding(vector)
    }
  }
}
