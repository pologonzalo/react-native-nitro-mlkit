...
Android-only packages (no iOS implementation) must be marked
`"private": true` in their `package.json` so they cannot be published by
accident. Drop the flag in the same PR that adds their iOS support.
